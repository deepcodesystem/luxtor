...
publicWidget.registry.LxInstallToggle = publicWidget.Widget.extend({
    ...
    _renderUnitOnly() {
        ...
        if (!el) return;
        const txt = (this._unitFormatted || "").replace(/[()]/g, "");
        el.textContent = txt;
...
publicWidget.registry.LxDeliveryPreviewAllCarriers = publicWidget.Widget.extend({
    ...
    init() {
        ...
        // Prevent flash: keep delivery form hidden until we decide free/not-free once.
    ...
    async _syncSelectedCarrierOnce() {
        ...
        if (this._lastSyncedCarrier === dmId) return;
    ...
    _applyFreeUi(isFree) {
        ...
        const tplMsg = document.getElementById("lx_free_delivery_msg"); // if you ever add it back later

        const existing = document.getElementById(msgId);
        if (existing) existing.remove();
        if (tplMsg) tplMsg.remove();
    ...
    _ensureStyles() {
        ...
        style.textContent = `
            /* Hard hide (prevents flash) */
            .lx-delivery-hidden { display: none !important; }

            /* Modern info banner like your screenshot (no icon) */
            .lx-info-banner{
                display:block;
                padding:14px 16px;
...
publicWidget.registry.LxHideFromAfterVariantChange = publicWidget.Widget.extend({
    ...
    start() {
        ...
        // Hide "From" only AFTER user interacts with variants/custom inputs
