/** @odoo-module **/
/**
 * LxDeliveryPreviewAllCarriers — Pre-computes and renders delivery prices on the
 *   checkout page before the user selects a carrier.
 *   Calls /lx/shipping/preview_prices and patches the Odoo price badges.
 *
 * LxHideFromAfterVariantChange — Hides the "From" price label after the user
 *   picks a variant or enters dimensions.
 */
...
function _dbgEnabled() {
    return false; // set to true or gate on URL param for local debugging
}
...
publicWidget.registry.LxHideFromAfterVariantChange = publicWidget.Widget.extend({
    ...
    start() {
        this._super(...arguments);
        this.$el.one('change', '.js_variant_change, input[name="lx_width_m"], input[name="lx_height_m"]', () => {
