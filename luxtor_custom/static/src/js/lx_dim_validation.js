...
publicWidget.registry.LxDimensionsLiveValidation = publicWidget.Widget.extend({
    ...
    _msgNodeFor(inputEl) {
        ...
        if (!node) {
            ...
            // extra spacing: mt-2, and keep small red text
    ...
    _clearFieldState(inputEl) {
        ...
        // also clear any previous constraint flag
    ...
    _validateNow() {
        ...
        // width: only validate if user entered a value
        if (wEl) {
            const raw = (wEl.value || "").trim();
            if (!raw) {
                // optional: empty means no error, no block
        ...
        // height: only validate if user entered a value
        if (hEl) {
        ...
        // If both fields are empty, do not block cart
