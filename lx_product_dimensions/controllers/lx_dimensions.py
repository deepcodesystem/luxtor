...
class LxDimensionsController(http.Controller):
    ...
    def _max_width_m_for_template(self, tmpl):
        ...
        if not fabric:
            ...
            if code:
                fabric = request.env["product.template"].sudo().search(
    ...
    def dimensions_hints(self, product_id=None, product_tmpl_id=None, width=None, unit="m"):
        ...
        max_h_m = (
            self._max_height_m_for_template(tmpl, width_m)
            if width_m > 0
            else self._max_height_m_for_template(tmpl, 1.0)
        )

        width_hint = f"min: {self._fmt_unit(min_w_m, 'm')}" + (
            f" / max: {self._fmt_unit(max_w_m, 'm')}" if max_w_m > 0 else ""
        )
        height_hint = f"min: {self._fmt_unit(min_h_m, 'm')}" + (
            f" / max: {self._fmt_unit(max_h_m, 'm')}" if max_h_m > 0 else ""
        )
